        ...
        <div class="w-full max-w-[95%] lg:max-w-[90%] mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-6 lg:gap-8 items-stretch">
                ...
                <div id="panel-reservacion"
                    ...
                    @if(!$reservasAbiertas)
                        <div
                            class="mb-6 p-4 bg-amber-50 border border-amber-200 text-amber-800 rounded-xl flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs mt-0.5">El registro de reservaciones diarias está programado para iniciar
                                    a las 8:00 a.m.</p>
                            </div>
                        </div>
                    @endif

                    <form id="reservacion-form" action="{{ route('reservaciones.store') }}" method="POST"
                        ...
                        <div class="space-y-3">
                            ...
                            <div
                                style="display: flex; flex-direction: row; gap: 8px; width: 100%; justify-content: space-between; align-items: stretch;">
                                <!-- 12:30 Button -->
                                <button type="button" @click="selectedHora = '12:30'"
                                    ...
                                    <span class="font-bold text-xs sm:text-sm truncate w-full block"
                                        :class="selectedHora === '12:30' ? 'text-white' : ''">12:30 p.m. a</p> 1:00
                                        p.m.</span>
                                    <span class="text-[11px] sm:text-xs truncate w-full block mt-0.5"
                                        :class="selectedHora === '12:30' ? 'text-indigo-100' : ''">
                                        {{ $horariosStatus['12:30']['reservados'] }}/{{ $horariosStatus['12:30']['capacidad'] }}
                                    </span>
                                </button>

                                <!-- 13:15 Button -->
                                <button type="button" @click="selectedHora = '13:15'"
                                    ...
                                    style="flex: 1 1 0%; min-width: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 12px 4px; border-radius: 16px; border-width: 1px; border-style: solid; text-align: center; transition: all 0.2s; min-height: 66px;">
                                    <span class="font-bold text-xs sm:text-sm truncate w-full block"
                                        :class="selectedHora === '13:15' ? 'text-white' : ''">1:15 p.m. a</p> 1:45
                                        p.m</span>
                                    <span class="text-[11px] sm:text-xs truncate w-full block mt-0.5"
                                        :class="selectedHora === '13:15' ? 'text-indigo-100' : ''">
                                        {{ $horariosStatus['13:15']['reservados'] }}/{{ $horariosStatus['13:15']['capacidad'] }}
                                    </span>
                                </button>

                                <!-- 14:00 Button -->
                                <button type="button" @click="selectedHora = '14:00'"
                                    ...
                                    style="flex: 1 1 0%; min-width: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 12px 4px; border-radius: 16px; border-width: 1px; border-style: solid; text-align: center; transition: all 0.2s; min-height: 66px;">
                                    <span class="font-bold text-xs sm:text-sm truncate w-full block"
                                        :class="selectedHora === '14:00' ? 'text-white' : ''">2:00 p.m. a</p> 2:30
                                        p.m.</span>
                                    <span class="text-[11px] sm:text-xs truncate w-full block mt-0.5"
                                        :class="selectedHora === '14:00' ? 'text-indigo-100' : ''">
                                        {{ $horariosStatus['14:00']['reservados'] }}/{{ $horariosStatus['14:00']['capacidad'] }}
                                    </span>
                                </button>

                                <!-- 14:45 Button -->
                                <button type="button" @click="selectedHora = '14:45'"
                                    ...
                                    style="flex: 1 1 0%; min-width: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 12px 4px; border-radius: 16px; border-width: 1px; border-style: solid; text-align: center; transition: all 0.2s; min-height: 66px;">
                                    <span class="font-bold text-xs sm:text-sm truncate w-full block"
                                        :class="selectedHora === '14:45' ? 'text-white' : ''">2:45 p.m. a</p> 3:15
                                        p.m.</span>
                                    <span class="text-[11px] sm:text-xs truncate w-full block mt-0.5"
                                        :class="selectedHora === '14:45' ? 'text-indigo-100' : ''">
                                        {{ $horariosStatus['14:45']['reservados'] }}/{{ $horariosStatus['14:45']['capacidad'] }}
                                    </span>
                                </button>

                                <!-- 15:30 Button (Acceso libre) -->
                                <button type="button" @click="selectedHora = '15:30'"
                                    ...
                                    style="flex: 1 1 0%; min-width: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 12px 4px; border-radius: 16px; border-width: 1px; border-style: solid; text-align: center; transition: all 0.2s; min-height: 66px;">
                                    <span class="font-bold text-xs sm:text-sm truncate w-full block"
                                        :class="selectedHora === '15:30' ? 'text-white' : ''">3:30 p.m. a</p> 4:30
                                        p.m.</span>
                                    <span
                                        class="text-[11px] sm:text-xs truncate w-full block mt-0.5 {{ old('hora') === '15:30' ? 'text-emerald-100' : 'text-emerald-600 font-bold' }}"
                                        :class="selectedHora === '15:30' ? 'text-emerald-100' : ''">
                                        Acceso libre
                                    </span>
                                </button>
                            </div>
